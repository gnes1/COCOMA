__includes ["Main.nls" "FactoryPlanConvoi.nls" "FactoryPlanEnnemis.nls" "FactoryPlanDrones.nls" "FactoryPlanGeneral.nls" "Balles.nls" "Decision.nls" "Objectif.nls" "animation.nls" "animationVolant.nls" "DeplacementGeneral.nls" "Environnement.nls" "Tests.nls" "cercle.nls" "Largeur.nls" "Message.nls" "MessageTypeConvoi.nls" "MessageTypeEnnemis.nls" "MessageTypeDrones.nls" "Convoi.nls"  "Ennemi.nls"  "Drone.nls" ]

;On peut preciser le nom du leader dans les messages pour faire un tri supplémentaire pour les messages et la cohésion du groupe
...
globals
[
  mapAlt solAlt basseAlt hauteAlt ; variables topologiques Z discretise: definit le niveau ou se trouvent toutes les informations de la carte (obstacles base etc.) car en 2D, niveau au sol ou se trouvent les agents, niveau basse altitude et niveau haute altitude
...
patches-own
[
  obstacle? base? hangar? objectif? bridge? montagne? riviere? ; variables topologiques au niveau mapAlt, permet de definir les patchs praticables et ceux qui sont des obstacles
...
[
  ...
  listeConvoiAPorteePerception
  listeEnnemiAPorteePerception
  listeDroneAPorteePerception

  listeEnnemiAPorteeCommunication
  listeConvoiAPorteeCommunication
  listeDroneAPorteeCommunication
  PremiereFois
  listeAdj
  listeCompletementPercu
  listePartiellementPercu
  nomAgent
  boiteALettre
  boiteALettreAccuserReception
  DetresseRessource?
  essenceMax
  essenceActuel

  typeObjectif
  typeObjectifSecondaire
  AppelDetresse?
  objectifs
  chemins
  cheminsOffset
  ...
  CleCryptageLeaderRandom
  NbAgentParticipantElectionRandom

  PointDeRalliement
  appelDetresseEnvoyer
  attaquer?

  listeElectionRandomKey
  listeElectionRandomSomme

  listePatchAPorteeImédiate
]
...
[
  ...
  listeConvoiAPorteeDeTir
  listeEnnemiAPorteeDeTir
  listeDroneAPorteeDeTir
...
;  setup-hq
  ...
  set LeSuiveur -1
  set as-path transformListePatchListeCoord item 0 as-path
...
print patchObjectif
  ...
  if nb-mountains > 0 [
    repeat nb-mountains [
      ...
      let taille random 9
      let colorOk false
      let emplacementOk false
      ;; Recherche des positions disponibles pour une montagne
      while [  not colorOk  or not emplacementOk]
      [
        set colorok false
        set emplacementOk false

        if x > 13 and y > 16
        [
         set emplacementOk true
        ]

        if  distanceManhattan (list x y 1) patchObjectif < 6
        [
          set emplacementOk false
        ]
        ask patch x y 0
        [
          if ((pcolor > 51 and pcolor  <= 59) or (pcolor > 61 and pcolor <= 69) ) and pcolor != 35
          [set colorOk true]

        ]
        if  not colorOk  or not emplacementOk
        [
          set x random 70
          set y random 70
        ]
      ...
      let listePatchMontagne cerclePerception3DS x y 0 (2 + random 3) true
      let i 0
      while [ i < length(listePatchMontagne) ]
      [
        let j 0
        while [ j < (1 + random 9) ]
        [
          ask patches with [pxcor = item 0 item i listePatchMontagne and pycor =  item 1 item i listePatchMontagne  and pzcor = j ]
          [
            ifelse random 2 = 0
            [
              set pcolor 31 + random 3
            ]
            [
              set pcolor 36 + random 3
            ]
...
to setup-convois
  if nb-cars > 0 [
    ...
    let base-min-pxcor 5
    let base-max-pxcor 6
    let base-min-pycor 1
    let base-max-pycor 11
    ...
    [
      ...
      set dead? false
      set PVMAX ConvoiPVMAX
      set PV PVMAX
      set PremiereFois true
      ...
      set leaderVivant? true
       ...
       set   AppelDetresse? false
      ; Visu
      set objectifs  patchObjectif
      ...
      set listeAdj []
      set listeCompletementPercu []
      set listePartiellementPercu []
      set nomAgent who
      set boiteALettre []
      set boiteALettreAccuserReception []

      set essenceMax ConvoiCapaciteReservoir
      ...
      set appelDetresseEnvoyer false
      set attaquer? false

      set listeElectionRandomKey []
      set listePatchAPorteeImédiate []

     if who = 0 [ set leader? true ]
    ]
    set pokecar item random length([who] of convois) [who] of convois
    ...
    if nb-cars > 1 [
      ...
      ifelse (who - 1) mod 2 = 0 [ set xconvoi base-min-pxcor ] [ set xconvoi base-max-pxcor ]

      let aux1Y base-max-pycor - base-min-pycor
      let aux2Y random aux1Y
      let yconvoi  base-min-pycor + aux2Y

      move-to patch  xconvoi yconvoi solAlt
      ]
...
to setup-ennemis
  if NbEnnemis > 0 [
    ; get the size of the base to deploy the car accordingly
    let base-ennemi-min-pxcor min-pxcor
    let base-ennemi-max-pxcor max-pxcor
    let base-ennemi-min-pycor min-pycor
    let base-ennemi-max-pycor max-pycor

    ; creation des voitures du ennemi et cortege
    create-ennemis NbEnnemis
    ask ennemis
    [
      ; Init apparence NetLogo
      set shape "car"
      ...
      set essenceMax EnnemiCapaciteReservoir
      set essenceActuel essenceMax

      set munitionMax EnnemiMunitionMax
...
;      set IsAttacke? false
;      set dijkstraPrecalculer? false
;      set dijkstraPrecalculer []
    ...
    if NbEnnemis > 1 [
      ask ennemis [
        ...
        let xennemie  0
        let yennemie 0
        let peutSortir? false
        while [ peutSortir?  = false ]
        [
          let aux1Y base-ennemi-max-pycor - base-ennemi-min-pycor
          let aux2Y random aux1Y
          set yennemie  base-ennemi-min-pycor + aux2Y

          let aux1X base-ennemi-max-pxcor - base-ennemi-min-pxcor
          let aux2X random aux1X
          set xennemie  base-ennemi-min-pycor + aux2X

          let nomEnvPatch 0
          if xennemie < 70 and xennemie > 0 and yennemie > 0 and yennemie < 70
          [
            set nomEnvPatch environnementPatch xennemie yennemie
            if nomEnvPatch != "Base" and nomEnvPatch != "Eau" and nomEnvPatch !="Montagne"
            [
              set peutSortir? true
            ]
          ]
          move-to patch  xennemie yennemie solAlt ]
      ]
    ]
  ]
end
...
to setup-drones
  if NbDrones > 0 [
    ; get the size of the base to deploy the car accordingly
    let hangar-drone-min-pxcor 3
    let hangar-drone-max-pxcor 4
    let hangar-drone-min-pycor 11
    let hangar-drone-max-pycor 0


    ; creation des voitures du ennemi et cortege
    create-drones NbDrones
    ask drones
    [
      ...
      set essenceMax DroneCapaciteReservoir
      set essenceActuel essenceMax

      set munitionMax DroneMunitionMax
      set munitionActuel munitionMax
...
;     set leader? true
;     set color orange
;     move-to patch 3 0 solAlt
;   ]
;
;    ; configure the last car as the critical one
;   ask drone last-drone [
;     set to-protect? true
;     set color yellow
;   ]
    ...
    if NbDrones > 1 [
      ...
      ifelse (who - 1) mod 2 = 0 [ set xdrone hangar-drone-min-pxcor ] [ set xdrone hangar-drone-max-pxcor ]

      let aux1Y hangar-drone-max-pycor - hangar-drone-min-pycor
      let aux2Y random aux1Y
      let ydrone  hangar-drone-min-pycor + aux2Y



      move-to patch  xdrone ydrone solAlt ]
      ; ask non leader cars
;      ask turtle-set sort-on [who] drones with [who > first-drone]
;      [
;
;        create-drone-link-to turtle (who - 1)
;        if who >= 4 and who mod 2 = 0 [ create-drone-link-with turtle (who - 3) ]
;
;        ; deploying
;        ifelse (who - 1) mod 2 = 0 [ set xcor hangar-drone-min-pxcor ] [ set xcor hangar-drone-max-pxcor ] ; a gauche ou a droite selon le nombre (pair ou impair respectivement)
;        set ycor hangar-drone-min-pycor - (floor (who / 2) / (NbVehiculeDrone / 2) * (hangar-drone-min-pycor - hangar-drone-max-pycor)) ; d'une rangee de plus en plus basse toutes les deux voitures
;        set zcor solAlt
;;      ]
;
;    ]
...
to setupCarteComplete2D
  ...
  [
    ...
    while [ j < max-pycor ]
    [
      if not [obstacle?] of patch i j 1 [

      let aux []

      ask patch i j 1 [ ask neighbors [ if pzcor = 1 [ set aux lput (list pxcor pycor pzcor) aux ] ] ]

      let k 0
      let auxi []
      while[ k < length(aux) ]
      [
        if voisinDirect3D  i j 1 item 0 item k aux item 1 item k aux item 2 item k aux and not [obstacle?] of patch item 0 item k aux item 1 item k aux item 2 item k aux


          [
            set auxi lput item k aux auxi
          ]
        set k k + 1
      ]

      set liste lput (list (list i j 1) auxi) liste
      ]
      set j j + 1
    ]
   set i i + 1
  ...
  set CarteComplete2D liste
end

to setupCarteComplete3D
  ...
  [
    ...
    [
      ...
      while[ g < max-pzcor ]
      [
        if not [obstacle?] of patch i j g [
        let aux []

        ask patch i j g [ ask neighbors [  set aux lput (list pxcor pycor pzcor) aux  ] ]

        let k 0
        let auxi []
        while[ k < length(aux) ]
        [
          if voisinDirect3D  i j g item 0 item k aux item 1 item k aux item 2 item k aux and not [obstacle?] of patch item 0 item k aux item 1 item k aux item 2 item k aux
          [
            set auxi lput item k aux auxi
          ]
          set k k + 1
        ]
        set liste lput (list (list i j g) auxi) liste
        ]
        set g g + 1
      ]
      set j j + 1
  ...
  set CarteComplete3D liste
end

;initialise les cartes completes
to setupCarteComplete
  setupCarteComplete2D
  setupCarteComplete3D
end

to Mourrir
   envoieMessageMourrir  listeEnnemiAPorteeCommunication
   envoieMessageMourrir  listeConvoiAPorteeCommunication
   envoieMessageMourrir listeDroneAPorteeCommunication
end
@#$#@#$#@
GRAPHICS-WINDOW
0
...
896
352
Essence Max( Chaque déplacement coûte 1 unité )
11
0.0
1
...
851
449
Munition Max (Chaque attaque 1 munition)
11
0.0
1
...
1067
155
Nb Déplacement max par décision
11
0.0
1
...
290
NIL
ask convoi 0 [ MainConvoiGlobale ]
NIL
1
T
OBSERVER
...
350
NIL
ask convoi 0\n[\nAgentDansCercleV2  xcor ycor zcor \"Convoi\" true\nlet path raccourciAstarPath\n\nsuivrepath path 0.0001\n]
T
1
T
OBSERVER
